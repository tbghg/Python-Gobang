...
class ChessAI():
    def __init__(self, chess_len):  # chess_len是15，传参记着传过来！
        self.len = chess_len
    ...
    # 把所有的空位全部传过来
    def genmove(self, board, turn): # 并没有怎么用到turn
        moves = []
        for y in range(self.len):
            for x in range(self.len):
                if board[y][x] == 0:
                    score = self.pos_score[y][x]  # x，y的顺序可能和我的不一样
                    moves.append((score, x, y))

        moves.sort(reverse=True) # 给它排一下序
        return moves

    def search(self, board, turn):
        moves = self.genmove(board, turn) # 调用上一个函数，获取空的位置，turn没用
        bestmove = None
    ...
    def evaluatePoint(self, board, x, y, mine, opponent):
        dir_offset = [(1, 0), (0, 1), (1, 1), (1, -1)]  # 方向从左到右
        for i in range(4):
    ...
    def getLine(self, board, x, y, dir_offset, mine, opponent):
        ...
        for i in range(9):
            ...
            if (tmp_x < 0 or tmp_x >= self.len or
                ...
                line[i] = opponent  # 若超出范围则设置为对方的棋子
            else:
